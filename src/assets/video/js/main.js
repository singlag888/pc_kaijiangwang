...
export function settingIsOpen(flag){
	ifopen = flag;
}
...
function checkposition() {
	let carposition = new Array();
	let carsequence = new Array();
...
export function startcountdown(timer,code) {
	curLotteryCode = code;
...
function countdowntimer(timer) {
	...
	if(timer <= 2) {
		if(curLotteryCode.indexOf('ft') != -1){
			$(".redlight").css("display", "block");
		}
	}
	if(timer == 2) {
		if(ifopen&&videoTools.ifsund()){
			$("#sound").attr("src", require('../sound/cuttime.mp3'));
...
function startanimation() {
	setTimeout(function() {
		if(ifopen&&videoTools.ifsund()){
			$("#sound").attr("src", require('../sound/staring.mp3'));
...
export function finishgame(finalresult) {
	setTimeout(function() {
		$("#sound").attr("src", require('../sound/over.mp3'));
		document.getElementById("sound").pause();
		if(ifopen && videoTools.ifsund()) {
			$("#sound").removeAttr("loop");
			document.getElementById("sound").play();
...
export function showcurrentresult(results) {
	var resultarry = results;
	for(var i = 0; i < 10; i++) {
		var posname = i + 1;
		var position = "#pos" + posname;
...
function winnerpage(results) {
	...
	for(var i = 0; i < 3; i++) {
		...
		if(curLotteryCode.indexOf('ft')!=-1){
			$(position).attr("src", require("../images/ft/winner" + resultarry[i] + ".png"));
		}else{
			$(position).attr("src", require("../images/sc/winner" + resultarry[i] + ".png"));
		}
